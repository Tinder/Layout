...
public final class ViewLayoutItem: LayoutItem {
    ...
    init(layoutItemView: UIView, superviewConstraints: @escaping SuperviewConstraints) {
...
extension LayoutItem {
    ...
    func addingSuperviewConstraints(_ constraints: @escaping SuperviewConstraints) -> LayoutItem {
        ViewLayoutItem(layoutItemView: layoutItemView) {
            self.superviewConstraints() + constraints()
        }
    }

    func adding(_ constraints: @escaping @autoclosure () -> [NSLayoutConstraint]) -> LayoutItem {
        addingSuperviewConstraints(constraints)
    }

    func adding(_ constraint: @escaping @autoclosure () -> NSLayoutConstraint) -> LayoutItem {
    ...
    public func size(width: CGFloat,
                     height: CGFloat,
                     priority: UILayoutPriority = .required) -> LayoutItem {
    ...
    ///   - priority: (optional) priority of constraint
    public func size(_ sizeValue: CGSize,
                     priority: UILayoutPriority = .required) -> LayoutItem {
    ...
    ///   - width: width constraint
    ///   - priority: (optional) priority of constraint
    public func size(is relation: ConstraintRelation = .equal,
                     width: CGFloat,
                     priority: UILayoutPriority = .required) -> LayoutItem {
        addingSuperviewConstraints {
    ...
    ///   - priority: (optional) priority of constraint
    public func square(_ length: CGFloat,
                       priority: UILayoutPriority = .required) -> LayoutItem {
        addingSuperviewConstraints {
            [
    ...
    public func aspectRatio(_ ratio: CGFloat,
                            priority: UILayoutPriority = .required) -> LayoutItem {
        aspectRatio(.constrainWidth(ratio),
                    priority: priority)
    }
    ...
    public func aspectRatio(_ aspectRatio: AspectRatioConstraint,
                            priority: UILayoutPriority = .required) -> LayoutItem {
        switch aspectRatio {
        case .constrainWidth(let widthToHeight):
            return adding(self.layoutItemView.width.constraint(equalTo: self.layoutItemView.height,
                                                               multiplier: widthToHeight))
        case .constrainHeight(let widthToHeight):
            guard widthToHeight > 0 else {
    ...
    ///   - priority: (optional) priority of constraint
    public func pin(insets: UIEdgeInsets = .zero, priority: UILayoutPriority = .required) -> LayoutItem {
        adding(
            self.layoutItemView.edgeConstraints(insetBy: insets)
    ...
    ///   - inset: inset of view
    ///   - priority: (optional) priority of constraint
    public func pin(_ inset: CGFloat,
                    priority: UILayoutPriority = .required) -> LayoutItem {
        pin(
    ...
    ///   - priority: (optional) priority of constraint
    public func pinToMargin(_ inset: CGFloat, priority: UILayoutPriority = .required) -> LayoutItem {
        pinToMargin(
            insets: .init(top: inset, leading: inset, bottom: inset, trailing: inset),
            priority: priority
        )
    }
    ...
    ///   - priority: (optional) priority of constraint
    public func center(offset: UIOffset = .zero,
                       priority: UILayoutPriority = .required) -> LayoutItem {
        adding(
            self.layoutItemView.centerConstraints(offsetBy: offset).withPriority(priority)
        )
    }
    ...
    ///   - offset: (optional) offset from superview
    ///   - priority: (optional) priority of constraint
    public func center(_ axis: Axis,
                       offset: CGFloat = 0,
    ...
    ///   - priority: (optional) priority of constraint
    public func pad(_ margin: CGFloat? = nil,
                    priority: UILayoutPriority = .required) -> LayoutItem {
    ...
    ///   - priority: (optional) priority of constraint
    public func to(_ attribute: NSLayoutConstraint.Attribute,
                   is relation: ConstraintRelation = .equal,
                   multiplier: CGFloat = 1.0,
                   _ constant: CGFloat = 0,
                   priority: UILayoutPriority = .required) -> LayoutItem {
        adding(
            self.layoutItemView.constraint(is: relation,
    ...
    ///   - constant: (optional) constant
    ///   - priority: (optional) priority of constraint
    public func to(_ attributes: [NSLayoutConstraint.Attribute],
                   _ constant: CGFloat = 0,
                   priority: UILayoutPriority = .required) -> LayoutItem {
        adding(
            self.layoutItemView.constraints(toSuperview: attributes,
                                            constant: constant)
                .withPriority(priority)
        )
    }
    ...
    ///   - constant: (optional) constant
    ///   - priority: (optional) priority of constraint
    public func toMargin(_ attribute: NSLayoutConstraint.Attribute,
                         is relation: ConstraintRelation = .equal,
                         multiplier: CGFloat = 1.0,
                         _ constant: CGFloat = 0,
                         priority: UILayoutPriority = .required) -> LayoutItem {
        adding(
            self.layoutItemView.constraint(for: attribute,
                                           is: relation,
    ...
    ///   - constant: (optional) constant
    ///   - priority: (optional) priority of constraint
    public func toMargin(_ attributes: [NSLayoutConstraint.Attribute],
                         _ constant: CGFloat = 0,
                         priority: UILayoutPriority = .required) -> LayoutItem {
        adding(
            attributes.map {
                self.layoutItemView.constraint(for: $0,
    ...
    ///   - priority: (optional) priority of constraint
    public func toBottomMargin(minInset: CGFloat,
                               priority: UILayoutPriority = .required) -> LayoutItem {
        addingSuperviewConstraints {
            guard let superview = self.layoutItemView.superview else { return [] }
            return [
                self.layoutItemView.constraint(for: .bottom,
                                               to: .bottomMargin,
                                               of: superview)
                    .withPriority(priority - min(1, priority.rawValue / 2)),
                self.layoutItemView.bottom.constraint(is: .lessThanOrEqual,
                                                      to: superview.bottom,
                                                      constant: -minInset)
                    .withPriority(priority)
            ]
    ...
    /// +---------------------+
    /// ```
    public func center(between top: NSLayoutYAxisAnchor,
                       and bottom: NSLayoutYAxisAnchor,
                       topOffset: CGFloat = 0,
                       bottomOffest: CGFloat = 0) -> LayoutItem {
        addingSuperviewConstraints {
            guard let superview = self.layoutItemView.superview else { return [] }
    ...
    /// +------------------------------+
    /// ```
    public func center(between leading: NSLayoutXAxisAnchor,
                       and trailing: NSLayoutXAxisAnchor,
                       leadingOffset: CGFloat = 0,
                       trailingOffest: CGFloat = 0) -> LayoutItem {
        addingSuperviewConstraints {
            guard let superview = self.layoutItemView.superview else { return [] }
            let guide = UILayoutGuide()
            superview.addLayoutGuide(guide)
...
extension NSLayoutConstraint.Attribute {
    ...
    var marginAttribute: NSLayoutConstraint.Attribute {
        switch self {
        case .left:
            return .leftMargin
        case .right:
...
extension LayoutItem {
    ...
    ///   - insets: (optional) insets of view
    ///   - priority: (optional) priority of constraint
    public func toGuides(insets: UIEdgeInsets = .zero,
                         priority: UILayoutPriority = .required) -> LayoutItem {
    ...
    ///   - inset: inset of view
    ///   - priority: (optional) priority of constraint
    public func toGuides(_ inset: CGFloat,
                         priority: UILayoutPriority = .required) -> LayoutItem {
        toGuides(insets: UIEdgeInsets(top: inset, left: inset, bottom: inset, right: inset), priority: priority)
    }
    ...
    ///   - constant: (optional) constant
    ///   - priority: (optional) priority of constraint
    public func toGuide(_ attribute: XAxisAttribute,
                        _ constant: CGFloat = 0,
                        priority: UILayoutPriority = .required) -> LayoutItem {
    ...
    ///   - constant: (optional) constant
    ///   - priority: (optional) priority of constraint
    public func toGuide(_ attribute: YAxisAttribute,
                        _ constant: CGFloat = 0,
                        priority: UILayoutPriority = .required) -> LayoutItem {
        toGuideAttribute(attribute, constant, priority: priority)
    }
    ...
    ///   - constant: (optional) constant
    ///   - priority: (optional) priority of constraint
    public func toGuide(_ attribute: DimensionAttribute,
                        _ constant: CGFloat = 0,
                        priority: UILayoutPriority = .required) -> LayoutItem {
        toGuideAttribute(attribute, constant, priority: priority)
    }

    func toGuideAttribute<T: AnchorAttribute>(_ attribute: T,
                                              _ constant: CGFloat = 0,
                                              priority: UILayoutPriority = .required) -> LayoutItem {
        addingSuperviewConstraints {
            guard let safeAnchor = self.safeAreaGuide?.anchor(for: attribute) else { return [] }
    ...
    private var safeAreaGuide: UILayoutGuide? {
        guard let superview = layoutItemView.superview else {
